...
def buy(symbol):
	...
	time1 = datetime.now()
	try:
		session.execute("INSERT INTO stocks.sales (id, symbol, price, user_name, share_name) VALUES (%s, %s, %s, %s, %s)", 
			[uuid_from_time(time1), quote["symbol"] , float(quote["latestPrice"]), sess['name'], quote["companyName"]])
...
def display_records():
	try:
		rows = session.execute("""SELECT * FROM stocks.sales WHERE user_name = '{}' ALLOW FILTERING""".format(sess['name']))
		result = []
		for row in rows:
			result.append(row)
...
def return_stocks(r_id):
	session.execute("""DELETE FROM stocks.sales WHERE id = {}""".format(r_id))	
